...
Setup guide for adding SSH-authentication, disabling authentication with credentials, installing Nginx, and connecting the server to your personal Git account. After completing this guide, you should be able to connect to a custom Nginx HTML template.
...
Check the config before restarting the server:
...
Paste the following config into the newly created file:
...
server {
        ...
        root /var/www/alternatives;             #Path to alternative .html file
        index alternate-index.html;             #Specifies which .html has to be used
...
Copy the output manually, then on the server add it to `~/.ssh/authorized_keys` using the steps in [Section 1](#1-ssh-key-generation-and-server-access).
